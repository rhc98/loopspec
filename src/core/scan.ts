...
const COMMAND_PATTERNS: Pattern[] = [
  ...
  { rule: "interpreter-eval", level: "danger", re: /\b(node|deno|bun|python3?|ruby|perl|php|osascript)\s+(-e|-c|-r|--eval|--exec)\b/, message: "runs inline code through an interpreter" },
  { rule: "script-exec", level: "danger", re: /(^|[;&|]\s*)(sh|bash|zsh|source|\.)\s+\S/, message: "executes or sources a script file" },
  ...
  { rule: "dev-network", level: "danger", re: /\/dev\/(tcp|udp)\//, message: "raw TCP/UDP socket via /dev/tcp (network egress)" },
  { rule: "destructive-delete", level: "danger", re: /\brm\s+(-[a-zA-Z]*[rf]|--recursive|--force)\b|\bfind\b[^|]*-delete\b|\bfind\b[^|]*-exec\s+rm\b/, message: "recursive or forced file deletion" },
  { rule: "disk-write", level: "danger", re: /\bdd\b\s|\bmkfs\b|>\s*\/dev\/(?!tcp\/|udp\/)/, message: "raw disk / device write" },
  ...
  { rule: "local-install", level: "warn", re: /\b(npm|pnpm)\s+(i|install|ci)\b|(^|[;&|]\s*)yarn(\s|$)|\byarn\s+(install|add)\b/, message: "local install runs dependency lifecycle scripts (postinstall)" },
  { rule: "git-hook", level: "warn", re: /\bgit\s+config\b|\bcore\.(hooksPath|pager|fsmonitor)\b|\bgit\s+-c\s/, message: "git config can hijack hooks / pager to run code" },
  { rule: "make-executable", level: "warn", re: /\bchmod\s+\+x\b/, message: "marks a file executable" },
